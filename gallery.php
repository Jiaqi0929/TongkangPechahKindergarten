...
$dbUser = "if0_39656443";            
$dbPass = "TPKindergarten";         
$dbName = "if0_39656443_Kindergarten"; 
...
// Build SQL query 
// GROUP_CONCAT combines multiple tags into one comma-separated string
// COALESCE Returns the first non-NULL image URL (activity image if exists, otherwise facility image)
$sql = "SELECT 
            gi.item_id,
            gi.title,
            gi.description, 
...
// Arrays to store WHERE conditions and their parameters
$where = [];
$params = [];

// Uses named parameter :category if not 'all'
if ($category !== 'all') {
...
// Adds WHERE clause with all conditions joined by AND
if (!empty($where)) {
...
// Prepare and execute query with all parameters
try {
