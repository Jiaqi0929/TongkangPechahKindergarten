...
try {
    ...
    function getActivityDetails($conn, $activityId) {
        ...
        $featureQuery->execute([$activityId]);
    ...
    // Get video URLs
    $weeklyVideo = $conn->query("SELECT video_url FROM activities WHERE type_id = 1 AND video_url IS NOT NULL LIMIT 1")->fetch(PDO::FETCH_ASSOC);
